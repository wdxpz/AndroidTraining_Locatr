...
//            urlConnection.setDoOutput(true);
 ...
 //           Log.d(TAG, jsonResutl);
    ...
    public List<GalleryItem> searchItem(Location location) {
        List<GalleryItem> itemList = new ArrayList<>();
        ...
        try {
            ...
            for(int i=0; i<jsonPhotoArry.length(); i++){
                ...
                item.setId(jsonPhotoObject.getString("id"));
                item.setUrl(jsonPhotoObject.getString("url_s"));
                item.setLatitude(jsonPhotoObject.getDouble("latitude"));
                item.setLongtitude(jsonPhotoObject.getDouble("longitude"));
                itemList.add(item);
            }
        } catch (JSONException e) {
            e.printStackTrace();
